...
{{ synth_column_unique_key(name='column_ukey', null_frac=0.2) }}
{{ synth_column_boolean(name="column_boolean", pct_true=0.2, null_frac=0.2) }}
{{ synth_column_integer(name="column_integer", min=2000, max=2020, null_frac=0.2) }}
{{ synth_column_integer_sequence(name="column_intseq", step=1, start=1) }}
{{ synth_column_numeric(name="column_numeric", min=1.99, max=999.99, precision=2, null_frac=0.2) }}
{{ synth_column_string(name="column_string", min_length=10, max_length=20, null_frac=0.2) }}
{{ synth_column_date(name="column_date", min='1938-01-01', max='2024-12-31', null_frac=0.2) }}
{{ synth_column_date_sequence(name="column_dateseq", start_date='2024-08-10', step=3)}}
{{ synth_column_timestamp(name="column_timestamp", min='1938-01-01 00:00:00', max='2024-12-31 23:59:59', null_frac=0.2) }}
...
{{ synth_column_values(name="column_values", values=['Mathematics', 'Science', 'English Language Arts', 'Social Studies'], probabilities=[0.2, 0.3, 0.15, 0.35]) }}
...
{{ synth_column_foreign_key(name='column_fkey', model_name='stores', column='k_store', null_frac=0.3) }}
...
{{ synth_column_select(name='column_select', model_name="synth_words", value_cols="word", distribution="weighted", weight_col="frequency", filter="part_of_speech like '%ADJ%'", null_frac=0.2) }}
...
{{ synth_column_word(name='column_word', language_code="en", distribution="weighted", pos=["NOUN", "VERB"], null_frac=0.2) }}
{{ synth_column_words(name='column_words1', language_code="en", distribution="uniform", n=5) }}
{{ synth_column_words(name='column_words2', language_code="en", distribution="uniform", format_strings=["{ADV} learning for {ADJ} {NOUN}s", "{ADV} {VERB} {NOUN} course"]) }}
{{ synth_column_language(name='column_language', type="name", distribution="weighted") }}
{{ synth_column_address(name='column_address', countries=['United States'], parts=['street_address', 'city', 'geo_region', 'country', 'postal_code']) }}
